...
def find_files(
    ...
    for path in path_or_path_list:
        ...
        if path.is_dir():
            wav_paths = sorted(file_path.as_posix() for file_path in path.rglob("*") if file_path.suffix.lower() == ".wav")
...
def load_wav(wav_path: str, sr: int = 16000) -> NDArray:
    ...
    y, source_sr = sf.read(wav_path, dtype="float32", always_2d=False)
    ...
    if source_sr != sr:
        try:
            ...
            raise ModuleNotFoundError(
                "Resampling audio features requires the 'soxr' package when the source sample rate differs from the requested sample rate."
            ) from exc

        if y.ndim == 1:
            y = soxr.resample(y, source_sr, sr, quality="HQ").astype(np.float32)
        else:
            y = np.stack(
                [soxr.resample(channel, source_sr, sr, quality="HQ").astype(np.float32) for channel in y],
                axis=0,
            )
