...
$buildDir = Join-Path $repoPath "build\hls\subband_q610_realtime_rtl_engine_coverage_hls\solution1\csim\build"
$exe = Join-Path $buildDir "csim.exe"
$logDir = Join-Path $repoPath "logs\hls\coverage_segments_rtl_engine"

if (-not (Test-Path $exe)) {
  throw "Missing csim.exe. Run run_hls_realtime_rtl_engine_csim_coverage.tcl once to build C-sim first: $exe"
}
...
for ($i = 0; $i -lt $ranges.Count; $i += $ParallelJobs) {
  $last = [Math]::Min($i + $ParallelJobs - 1, $ranges.Count - 1)
  $batch = $ranges[$i..$last]
  $jobs = @()
